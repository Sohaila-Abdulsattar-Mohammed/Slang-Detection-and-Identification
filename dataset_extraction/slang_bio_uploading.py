...
# Load the dataset
file_path = "bio_tagged_dataset_final.csv"
dataset = pd.read_csv(file_path)

# Clean the word_labels column by stripping spaces around labels
def clean_labels(label_str):
    return ','.join(label.strip() for label in label_str.split(','))

dataset['word_labels'] = dataset['word_labels'].apply(clean_labels)
...
dataset_name = "SohailaMohammed/BERTSlangIdentification"
